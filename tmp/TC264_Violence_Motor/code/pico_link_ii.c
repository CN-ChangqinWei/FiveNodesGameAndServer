...
#define UART_SEND_BYTES(array, length) uart_write_buffer((UART_3), (array), (length))
//----------------------SPI 宏定义-----------------------------
// SPI一次传输最大字节数, 最大16384, 对于双向全双工SPI最大4096
#define SPI_MAX_TRANS_SIZE (16384)
// SPI CS引脚拉低, 需要自己找函数定义
#define SPI_CS_LOW gpio_set_level(P20_13, 0)
// SPI CS引脚拉高, 需要自己找函数定义
#define SPI_CS_HIGH gpio_set_level(P20_13, 1)
// 全双工SPI发送多个字节, 需要自己找函数定义, array:发送数据首地址, length:传输长度
#define SPI_SEND_BYTES(array, length) spi_write_8bit_array((SPI_0), (array), (length))
// 全双工SPI读取多个字节, 需要自己找函数定义, buffer:接收数据首地址, length:传输长度
#define SPI_READ_BYTES(buffer, length) spi_read_8bit_array((SPI_0), (buffer), (length))
