...
void ips200_clear_region(uint16 start_x,uint16 start_y,uint16 end_x,uint16 end_y)
{
    uint16 i, j;
    if(IPS200_TYPE_SPI == ips200_display_type)
    {
...
// 备注信息
//-------------------------------------------------------------------------------------------------------------------
void ips200_show_hex (uint16 x, uint16 y, const uint32 dat, uint8 num)
{
    // 如果程序在输出了断言信息 并且提示出错位置在这里
    // 那么一般是屏幕显示的时候超过屏幕分辨率范围了
    zf_assert(x < ips200_x_max);
    ...
    func_hex_to_str(data_buffer, dat_temp);
    ips200_show_string(x, y, (const char *)&data_buffer);
}
